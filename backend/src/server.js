...
const gisUtils_1 = require("./gisUtils");
const dotenv_1 = __importDefault(require("dotenv"));
const axios_1 = __importDefault(require("axios"));
dotenv_1.default.config();
const app = (0, express_1.default)();
app.use((0, cors_1.default)()); // 운영 환경 배포를 위해 모든 도메인 허용
...
const authMiddleware = (req, res, next) => {
    ...
    if (clientAuth === masterPassword) {
        ...
        console.warn(`[AUTH] Unauthorized access attempt from ${req.ip}. Expected length: ${masterPassword.length}, Received length: ${(clientAuth === null || clientAuth === void 0 ? void 0 : clientAuth.length) || 0}`);
...
app.post('/api/login', (req, res) => {
    ...
    if (password === masterPassword) {
        ...
        console.warn(`[AUTH] Login Failed from ${req.ip}. Pwd: ${password} (len: ${password === null || password === void 0 ? void 0 : password.length}), Expected len: ${masterPassword.length}`);
...
function startServer() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            // Migration
            yield (0, database_1.runQuery)('UPDATE convenience_stores SET origin_lat = lat, origin_lng = lng WHERE origin_lat IS NULL');
            yield (0, database_1.runQuery)('UPDATE tobacco_shops SET origin_lat = lat, origin_lng = lng WHERE origin_lat IS NULL');
            // Serve
            ...
            // Background task
            (0, gisUtils_1.initRoadGraph)().catch(e => console.error('[GIS] Background init failed:', e.message));
        }
        catch (err) {
...
app.get('/api/stores', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const stores = yield (0, database_1.getQuery)('SELECT * FROM convenience_stores');
    res.json(stores);
}));
app.get('/api/tobacco', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const shops = yield (0, database_1.getQuery)('SELECT * FROM tobacco_shops');
    res.json(shops);
}));
app.get('/api/targets', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const targets = yield (0, database_1.getQuery)('SELECT * FROM target_points');
    res.json(targets);
}));
app.post('/api/targets', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { name, address, area_size, rent_fee, memo, lat, lng } = req.body;
    const result = yield (0, database_1.runQuery)(`INSERT INTO target_points (name, address, area_size, rent_fee, memo, lat, lng) VALUES (?, ?, ?, ?, ?, ?, ?)`, [name, address, area_size, rent_fee, memo, lat, lng]);
    res.json({ success: true, id: result.lastID });
}));
app.delete('/api/targets/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    yield (0, database_1.runQuery)(`DELETE FROM target_points WHERE id=?`, [req.params.id]);
    res.json({ success: true });
}));
app.post('/api/memos', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { entity_type, entity_id, memo } = req.body;
    yield (0, database_1.runQuery)(`INSERT INTO entity_memos (entity_type, entity_id, memo) VALUES (?, ?, ?)`, [entity_type, entity_id, memo]);
    res.json({ success: true });
}));
app.get('/api/memos', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { type, id } = req.query;
    const memos = yield (0, database_1.getQuery)(`SELECT * FROM entity_memos WHERE entity_type=? AND entity_id=? ORDER BY id DESC`, [type, id]);
    res.json(memos);
}));
app.put('/api/memos/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { memo } = req.body;
    yield (0, database_1.runQuery)(`UPDATE entity_memos SET memo=? WHERE id=?`, [memo, req.params.id]);
    res.json({ success: true });
}));
app.delete('/api/memos/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    yield (0, database_1.runQuery)(`DELETE FROM entity_memos WHERE id=?`, [req.params.id]);
    res.json({ success: true });
}));
app.get('/api/areas', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { mode } = req.query; // 'on' | 'invert'
        const allStores = yield (0, database_1.getQuery)("SELECT name, lat, lng FROM convenience_stores WHERE lat IS NOT NULL AND lng IS NOT NULL");
        const stores = allStores.filter((s) => !(s.name || '').includes('폐점'));
        const tobacco = yield (0, database_1.getQuery)('SELECT name, lat, lng FROM tobacco_shops WHERE lat IS NOT NULL AND lng IS NOT NULL');
        ...
        if (mode === 'invert') {
            const availableArea = (0, gisUtils_1.createAvailableArea)(limitPoints);
            res.json(availableArea);
        }
        else {
            const restrictedArea = (0, gisUtils_1.createRestrictedArea)(limitPoints);
...
app.get('/api/stats', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const storeRows = yield (0, database_1.getQuery)("SELECT brand_code, count(*) as cnt, sum(case when expiration_date <= '2027-12-31' then 1 else 0 end) as exp_cnt FROM convenience_stores WHERE name NOT LIKE '%폐점%' GROUP BY brand_code");
    const tobacco = yield (0, database_1.getQuery)("SELECT name FROM tobacco_shops");
    ...
    tobacco.forEach(sh => {
        ...
        if (isExcl && !isExpl)
            return;
        if (n.includes('CU') || n.includes('씨유'))
            bc = 'C';
        else if (n.includes('지에스') || n.includes('GS') || n.includes('gs'))
            bc = 'G';
        else if (n.includes('세븐'))
            bc = 'S';
        else if (n.includes('이마트'))
            bc = 'E';
        else if (n.includes('씨스페이스'))
            bc = 'P';
        if (bc)
            tobaccoStats[bc] = (tobaccoStats[bc] || 0) + 1;
    });
    // 기존 편의점 데이터와 합산
    ...
    // 타겟(Target) 후보지 개수 추가
    const targetCountRes = yield (0, database_1.getQuery)("SELECT count(*) as cnt FROM targets");
    const targetCount = ((_a = targetCountRes[0]) === null || _a === void 0 ? void 0 : _a.cnt) || 0;
...
app.get('/api/reverse-geocode', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (data.results && data.results.length > 0) {
            ...
            if (r.land && r.land.number1)
                address += ` ${r.land.number1}${r.land.number2 ? '-' + r.land.number2 : ''}`;
        }
...
app.get('/api/pano-proxy', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { lat, lng } = req.query;
    if (!lat || !lng)
        return res.status(400).json({ error: 'Lat/Lng required' });
    // Truncate lat/lng to 6 decimals (Naver API safety)
    const tLat = parseFloat(lat).toFixed(6);
    const tLng = parseFloat(lng).toFixed(6);
    try {
...
app.post('/api/stores/update-coords', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id, lat, lng } = req.body;
    yield (0, database_1.runQuery)(`UPDATE convenience_stores SET lat=?, lng=? WHERE id=?`, [lat, lng, id]);
    (0, gisUtils_1.clearAreaCache)();
    res.json({ success: true });
}));
// 🏢 건축물대장(위반건축물) 조회 프록시
app.get('/api/building-ledger', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j;
    const { lat, lng, sigunguCd: pSigungu, bjdongCd: pBjdong, bun: pBun, ji: pJi } = req.query;
    if (!lat || !lng)
        return res.status(400).json({ error: 'Lat/Lng required' });
    try {
        let sigunguCd = pSigungu;
        let bjdongCd = pBjdong;
        let bun = pBun || '0000';
        let ji = pJi || '0000';
        ...
        if (!sigunguCd || !bjdongCd) {
            ...
            const naverRes = yield axios_1.default.get(naverUrl, {
                headers: {
                    'X-NCP-APIGW-API-KEY-ID': process.env.NCP_CLIENT_ID || '',
                    'X-NCP-APIGW-API-KEY': process.env.NCP_CLIENT_SECRET || ''
                }
            ...
            const legalResult = naverData.results.find((r) => r.name === 'legalcode');
            const addrResult = naverData.results.find((r) => r.name === 'addr');
            if (!legalResult || !((_a = legalResult.code) === null || _a === void 0 ? void 0 : _a.id)) {
            ...
            if (addrResult === null || addrResult === void 0 ? void 0 : addrResult.land) {
        ...
        if (!serviceKey)
            return res.json({ success: false, message: 'API 인증키 누락' });
        ...
        for (const endpoint of endpoints) {
            ...
            try {
                const response = yield axios_1.default.get(publicUrl, { timeout: 10000 });
                const data = response.data;
                if (((_c = (_b = data.response) === null || _b === void 0 ? void 0 : _b.header) === null || _c === void 0 ? void 0 : _c.resultCode) === '00' && ((_f = (_e = (_d = data.response) === null || _d === void 0 ? void 0 : _d.body) === null || _e === void 0 ? void 0 : _e.items) === null || _f === void 0 ? void 0 : _f.item)) {
                    const itemData = data.response.body.items.item;
                    const items = Array.isArray(itemData) ? itemData : [itemData];
                    allItems = [...allItems, ...items];
                    console.log(`[LEDGER] [${endpoint}] Found ${items.length} items.`);
                }
                else if ((_h = (_g = data.response) === null || _g === void 0 ? void 0 : _g.header) === null || _h === void 0 ? void 0 : _h.resultCode) {
        ...
        const failedUrl = ((_j = err.config) === null || _j === void 0 ? void 0 : _j.url) ? `(URL: ${err.config.url.split('serviceKey=')[0]}...)` : '';
        res.json({ success: false, message: `조회 오류: ${err.message} ${failedUrl}` });
    }
}));
app.post('/api/entities/reset-coords', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { type, id } = req.body;
    const table = type === 'store' ? 'convenience_stores' : 'tobacco_shops';
    yield (0, database_1.runQuery)(`UPDATE ${table} SET lat = origin_lat, lng = origin_lng WHERE id = ?`, [id]);
    const updated = yield (0, database_1.getQuery)(`SELECT lat, lng FROM ${table} WHERE id = ?`, [id]);
    res.json({ success: true, coords: updated[0] });
}));
app.post('/api/entities/update-name', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { type, id, name } = req.body;
    const table = type === 'store' ? 'convenience_stores' : 'tobacco_shops';
    yield (0, database_1.runQuery)(`UPDATE ${table} SET name = ? WHERE id = ?`, [name, id]);
    res.json({ success: true });
}));
app.post('/api/entities/toggle-target', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { type, id, is_target } = req.body;
    const table = type === 'store' ? 'convenience_stores' : 'tobacco_shops';
    yield (0, database_1.runQuery)(`UPDATE ${table} SET is_target = ? WHERE id = ?`, [is_target ? 1 : 0, id]);
    res.json({ success: true });
}));
